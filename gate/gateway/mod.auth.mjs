...
fastify.register((instance, opts, next) => {
  instance.addHook('preHandler', async (req, res) => { const r = await new Promise(resolve => cdv.send({ type: 'checkDataIsValid', params: { body: req.body } }, resolve)); if (r.error) throw r }); // middleware below request
  //instance.addHook('preHandler', async (req, res) => { const r = await new Promise(resolve => sal.send({ type: 'sendActivateLink', params: { body: req.body } }, resolve)); if (r.error) throw r }); // middleware below request
